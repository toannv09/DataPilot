# DataPilot — Ứng dụng LLM/GenAI trong tự động khám phá dữ liệu và phát triển mô hình học máy

**Hệ thống ứng dụng LLM/GenAI hỗ trợ khám phá dữ liệu và phát triển mô hình học máy qua giao tiếp tiếng Việt tự nhiên.**
...
<img width="1882" height="905" alt="image" src="https://github.com/user-attachments/assets/b903c83c-a0a7-4785-9a87-3359ee633061" />
...
<img width="1083" height="385" alt="image" src="https://github.com/user-attachments/assets/3592cd6b-3081-41b8-81c0-40688791dced" />
...
<img width="472" height="536" alt="image" src="https://github.com/user-attachments/assets/5c6b5aa2-4618-4b96-b1af-1980759aa8b9" />
...
Mỗi phiên chạy gồm nhiều **bước thực thi** (tool calls do LLM lập kế hoạch). Execution Pass Rate đo tỷ lệ bước chạy thành công trên tổng số bước được gọi — phản ánh mức độ ổn định của pipeline trong thực tế, không phải trên test case nhân tạo.

```
Step-level Pass Rate = số bước thành công / tổng số bước × 100%
                     = 1.530 / 1.547 × 100% = 98,9%

Session Pass Rate    = số phiên không có bước lỗi nào / tổng phiên × 100%
                     = 89 / 114 × 100% = 78,1%
```
...
| Chi phí ước tính (gpt-5.4-nano) | ~$0,005 – $0,010/phiên |

Phân phối ổn định qua các lần đo (dao động 9.385–9.464 token), chi phí không phụ thuộc nhiều vào nội dung câu hỏi cụ thể.

Ước tính chi phí dựa trên giá gpt-5.4-nano: input $0,20/1M token, output $1,25/1M token — với ~9.425 token/phiên (split input/output khoảng 60/40) tổng chi phí ≈ $0,005–$0,010/phiên.

### Độ đầy đủ phân tích (EDA Completeness)

Checklist 8 hạng mục của một báo cáo EDA đầy đủ:

| # | Hạng mục | Đảm bảo bởi |
|---|---|---|
| 1 | Thống kê cơ bản (min/max/mean/std/median toàn bộ cột số) | Phase 1 — luôn chạy |
| 2 | Kiểm tra missing values (tỷ lệ, phân phối theo cột) | Phase 1 — luôn chạy |
| 3 | Phát hiện ngoại lai (outlier detection) | Phase 1 — luôn chạy |
| 4 | Correlation matrix (tương quan giữa các cột số, kèm p-value) | Phase 1 — luôn chạy |
| 5 | Biểu đồ phân phối / đa biến (histogram, scatter) | Phase 1 — trigger rule |
| 6 | Pattern thời gian (trend, seasonality) | Phase 2 — phụ thuộc LLM |
| 7 | Nhận xét tổng hợp bằng tiếng Việt | Phase 2 — phụ thuộc LLM |
| 8 | Gợi ý bước phân tích tiếp theo | Phase 2 — phụ thuộc LLM |

**5/8 mục đầu được đảm bảo cứng bởi kiến trúc** — nghĩa là dù LLM lập kế hoạch sai hoàn toàn, người dùng vẫn luôn có đủ 5 hạng mục cơ bản này trong mỗi phiên phân tích.

### Kiểm chứng giả thuyết

<img width="1760" height="290" alt="image" src="https://github.com/user-attachments/assets/9f4ea5eb-adea-48c4-b4c8-ab923ac06d40" />
...
<img width="1776" height="577" alt="image" src="https://github.com/user-attachments/assets/bbc368ba-6481-4362-82f5-0ee4f414e7c2" />
<img width="1762" height="276" alt="image" src="https://github.com/user-attachments/assets/06c6f7b9-ca3f-445a-a0e8-decf9ecbc1ce" />
